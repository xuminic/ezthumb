...
int crc_main(void *rtime, int argc, char **argv)
{
	...
	cslog("RUN1: CRC32=%X  CRC16=%X  CRC-CCITT=%X  CRC8=%X\n", 
			c32, c16, cci, c8);
	...
	cslog("RUN2: CRC32=%X  CRC16=%X  CRC-CCITT=%X  CRC8=%X\n", 
			c32, c16, cci, c8);
	...
	cslog("RUN3: CRC32=%X  CRC16=%X  CRC-CCITT=%X  CRC8=%X\n", 
			c32, c16, cci, c8);
	...
	cci = csc_crc_ccitt(0, testbuf, sizeof(testbuf));
	c8  = csc_crc8(0, testbuf, sizeof(testbuf));
	cslog("RUN4: CRC32=%X  CRC16=%X  CRC-CCITT=%X  CRC8=%X\n", 
			c32, c16, cci, c8);
	return 0;
}
